...
def add_course(): # new window definition
    def add_query():
        global root
        stat="INSERT INTO COURSE(coursename,courseid) VALUES ('"+E1.get()+"','"+E2.get()+"')"
        cur.execute(stat)
        con.commit()
...
def update_data(): # new window definition
    def UPDD():
        global root
        stat="UPDATE COURSES SET COURSENAME = '"+E1.get()+"' WHERE COURSEID ='"+E2.get()+"'"
        con.commit()
        cur.execute(stat)
        con.commit()
        add.config(state=NORMAL)
...
def del_data():
    def delete():
        global root
        stat="DELETE FROM COURSE WHERE COURSEID='"+E1.get()+"'"
